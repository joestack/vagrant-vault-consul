...
consul_version = ENV['CONSUL_VERSION'] || "1.9.4"
...
Vagrant.configure("2") do |config|
  ...
  if (consul_install)
    ...
    # Setup Consul user
    config.vm.provision "shell", path: "setup-user.sh",
      env: {
        "GROUP" => consul_group,
        "USER" => consul_user,
    ...
    config.vm.provision "shell", path: "install-consul-systemd.sh"
  end
  ...
  # Install Vault
  config.vm.provision "shell", path: "install-vault.sh",
    env: {
  ...
  config.vm.provision "shell", path: "install-vault-systemd.sh"
